...
def _get_phi_al_regional_JPN(C, mag, vs30measured, rrup):
    ...
    idx = (rrup <= 80.0) & (rrup >= 30.0)
    phi_al[idx] *= C["s5"] + (C["s6"] - C["s5"]) / 50.0 * (rrup[idx] - 30.0)
...
def _get_basic_term(C, ctx):
    ...
    c4m[ctx.mag < 4] = 1.0
    R = np.sqrt(ctx.rrup**2.0 + c4m**2.0)
    # basic form
    ...
    base_term[after] += (
        C["a5"] * (ctx.mag[after] - C["m1"])
        + C["a8"] * (8.5 - ctx.mag[after]) ** 2.0
        + (C["a2"] + C["a3"] * (ctx.mag[after] - C["m1"])) * np.log(R[after])
    )
    base_term[within] += (
        C["a4"] * (ctx.mag[within] - C["m1"])
        + C["a8"] * (8.5 - ctx.mag[within]) ** 2.0
        + (C["a2"] + C["a3"] * (ctx.mag[within] - C["m1"])) * np.log(R[within])
    )
    base_term[before] += (
        C["a4"] * (CONSTS["m2"] - C["m1"])
        + C["a8"] * (8.5 - CONSTS["m2"]) ** 2.0
        + C["a6"] * (ctx.mag[before] - CONSTS["m2"])
        + C["a7"] * (ctx.mag[before] - CONSTS["m2"]) ** 2.0
        + (C["a2"] + C["a3"] * (CONSTS["m2"] - C["m1"])) * np.log(R[before])
    )
    return base_term
...
def _get_derivative(C, sa1180, vs30):
    ...
    derAmp[idx] = (
        ...
        * (
            -1.0 / (sa1180[idx] + c)
            + 1.0 / (sa1180[idx] + c * (vs30[idx] / C["vlin"]) ** n)
        )
    )
    return derAmp
...
def _get_faulting_style_term(C, ctx):
    ...
    f7 = C["a11"] * np.clip(ctx.mag - 4.0, 0.0, 1.0)
    f8 = C["a12"] * np.clip(ctx.mag - 4.0, 0.0, 1.0)
    # ranges of rake values for each faulting mechanism are specified in
    # table 2, page 1031
    return f7 * ((ctx.rake > 30) & (ctx.rake < 150)) + f8 * (
        (ctx.rake > -150) & (ctx.rake < -30)
    )
...
def _get_inter_event_std(C, mag, sa1180, vs30):
    ...
    """
    tau_al = C["s3"] + (C["s4"] - C["s3"]) / 2.0 * (mag - 5.0)
    tau_al[mag < 5.0] = C["s3"]
    tau_al[mag >= 7.0] = C["s4"]
    tau_b = tau_al
...
def _get_intra_event_std(region, C, mag, sa1180, vs30, vs30measured, rrup):
    """
    Returns Phi as described at pages 1046 and 1047
    """
    phi_al = (
        _get_phi_al_regional_JPN(C, mag, vs30measured, rrup)
        if region == "JPN"
        else _get_phi_al_regional(C, mag, vs30measured, rrup)
    )
...
def _get_site_response_term(C, imt, vs30, sa1180):
    ...
    site_resp_term[gt_vlin] = (C["a10"] + C["b"] * CONSTS["n"]) * np.log(
        vs30_rat[gt_vlin]
    )
    ...
    site_resp_term[lw_vlin] = (
        ...
        + C["b"]
        * np.log(sa1180[lw_vlin] + C["c"] * vs30_rat[lw_vlin] ** CONSTS["n"])
    )
...
def _get_soil_depth_term(region, C, z1pt0, vs30):
    ...
    f2 = interpolate.interp1d(
        ...
        [C["a43"], C["a43"], C["a44"], C["a45"], C["a46"], C["a46"], C["a46"]],
        kind="linear",
    )
...
def _get_stddevs(region, C, imt, ctx, sa1180):
    ...
    """
    std_intra = _get_intra_event_std(
        region, C, ctx.mag, sa1180, ctx.vs30, ctx.vs30measured, ctx.rrup
    )
...
def _get_top_of_rupture_depth_term(C, imt, ctx):
    ...
    """
    return C["a15"] * np.clip(ctx.ztor / 20.0, None, 1.0)
    ...
    if region == "JPN":
        return (
            ...
            * np.exp(
                -5.23
                / 2.0
                * np.log((vs30**2 + 412.0**2.0) / (1360.0**2 + 412**2.0))
            )
        )
    return (1.0 / 1000.0) * np.exp(
...
def _hw_taper1(ctx):
    # Compute taper t1
    T1 = (90.0 - ctx.dip) / 45.0
    T1[ctx.dip <= 30.0] = 60.0 / 45.0
    return T1
...
def _hw_taper2(ctx):
    ...
    a2hw = 0.2
    T2 = 1.0 + a2hw * (ctx.mag - 6.5) - (1.0 - a2hw) * (ctx.mag - 6.5) ** 2
    T2[ctx.mag > 6.5] = 1.0 + a2hw * (ctx.mag[ctx.mag > 6.5] - 6.5)
    T2[ctx.mag <= 5.5] = 0.0
    return T2
...
def _hw_taper4(ctx):
    ...
    T4[ctx.ztor <= 10.0] = 1.0 - ctx.ztor[ctx.ztor <= 10.0] ** 2.0 / 100.0
    return T4
...
def _hw_taper5(ctx):
    ...
    ry1 = ctx.rx * np.tan(np.radians(20.0))
    idx = (ctx.ry0 - ry1) <= 0.0
    T5[idx] = 1.0
    idx = ((ctx.ry0 - ry1) > 0.0) & ((ctx.ry0 - ry1) < 5.0)
    T5[idx] = 1.0 - (ctx.ry0[idx] - ry1[idx]) / 5.0
    return T5
...
def _get_sa_at_1180(region, C, imt, ctx):
    ...
    vs30_1180 = np.ones_like(ctx.vs30) * 1180.0
    ...
    fake_z1pt0 = np.ones_like(ctx.vs30) * -1.0
    return (
        _get_basic_term(C, ctx)
...
class AbrahamsonEtAl2014(GMPE):
    ...
    def __init__(self, sigma_mu_epsilon=0.0, region=None):
        self.region = region
        assert self.region in (None, "CHN", "JPN", "TWN"), self.region
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        mean = (
            ...
            + _get_top_of_rupture_depth_term(C, imt, ctx)
            + _get_faulting_style_term(C, ctx)
            + _get_soil_depth_term(self.region, C, ctx.z1pt0, ctx.vs30)
        )

        mean += _get_regional_term(
            self.region, C, imt, ctx.vs30, ctx.rrup
        )
        
        mean += (self.sigma_mu_epsilon*get_epistemic_sigma(ctx))

        # get standard deviations
        sig, tau, phi = _get_stddevs(
...
class AbrahamsonEtAl2014RegCHN(AbrahamsonEtAl2014):
    ...
    def __init__(self, sigma_mu_epsilon=0.0):
        super().__init__(region="CHN", sigma_mu_epsilon=sigma_mu_epsilon)


class AbrahamsonEtAl2014RegJPN(AbrahamsonEtAl2014):

    def __init__(self, sigma_mu_epsilon=0.0):
        super().__init__(region="JPN", sigma_mu_epsilon=sigma_mu_epsilon)


class AbrahamsonEtAl2014RegTWN(AbrahamsonEtAl2014):

    def __init__(self, sigma_mu_epsilon=0.0):
        super().__init__(region="TWN", sigma_mu_epsilon=sigma_mu_epsilon)
