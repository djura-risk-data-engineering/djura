...
from .akkar_bommer_2010_swiss_coeffs import (
...
class AkkarBommer2010(GMPE):
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        sig = np.log(10 ** C['SigmaTot'])
        tau = np.log(10 ** C['tau'])
        phi = np.log(10 ** C['Sigma1'])
...
class AkkarBommer2010SWISS01(AkkarBommer2010):
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        """
        ctx = ctx.copy()
        ctx.vs30 = np.ones_like(ctx.vs30) * 600.
        ...
        _apply_adjustments(
            ...
            mean, sig, tau, phi, ctx, ctx.rjb, imt, log_phi_ss)
        sig = np.log(10 ** sig)
        tau = np.log(10 ** tau)
        phi = np.log(10 ** phi)
