...
def _check_cm_ck(kyushu, honshu, ctx):
    ...
    """
    tmp = np.array([
        kyushu.crosses(LineString([(hypo_lon, hypo_lat), (lon, lat)]))
        for hypo_lon, hypo_lat, lon, lat in
        zip(ctx.hypo_lon, ctx.hypo_lat, ctx.lon, ctx.lat)])
    tmp_1 = np.array([
        honshu.crosses(LineString([(hypo_lon, hypo_lat), (lon, lat)]))
        for hypo_lon, hypo_lat, lon, lat in
        zip(ctx.hypo_lon, ctx.hypo_lat, ctx.lon, ctx.lat)])
    ck = tmp.astype(int)
    cm = tmp_1.astype(int)
...
def _compute_volcanic_distances(ctx, kyushu, honshu):
    ...
    for m, k, lon, lat in zip(cm, ck, ctx.lon, ctx.lat):
        if m == 1:  # the site is in either the forearc or back arc of Honshu
            ...
            # the source to site path does not cross any volcanic front but
            # the site can still be in the backarc of honshu or forearc of
            # Kyushu volcanic front
            tmp = sg.Point(lon, lat).within(buffer_region)
            tmp_1 = sg.Point(lon, lat).within(buffer_region_1)
            if tmp:
                # check if the site is in the backarc region of honshu
                rrup_b = honshu.distance(sg.Point(lon, lat))
                rrup_f = ctx.rrup - rrup_b
            elif tmp_1:
                # check if the site is in the forearcarc region of Kyushu
                rrup_f = kyushu.distance(sg.Point(lon, lat))
                rrup_b = ctx.rrup - rrup_f
            else:
                # a neutral case wherein the site is not located in the
                # vicinity of either of the volcanic fronts
                rrup_f = 0
                rrup_b = 0
...
class BahrampouriEtAl2021Asc(GMPE):
    ...
    def get_mean_and_stddevs(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        """
...
class BahrampouriEtAl2021SSlab(GMPE):
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        C = self.COEFFS[imt]
        # Implements mean model (equation 12)
        mean = (_get_arias_intensity_term(ctx, C, trt)
                + _get_arias_intensity_second_term(ctx, C, trt))
