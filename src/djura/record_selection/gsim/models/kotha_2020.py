...
from .. import const
from ..imt import PGA, PGV, SA, IMT, from_string
from .nga_east import ITPL, TAU_SETUP, TAU_EXECUTION, get_tau_at_quantile
from ..utils import CallableDict

CONSTANTS = {"Mref": 4.5, "Rref": 30., "Mh": 5.7,
             "h_D10": 4.0, "h_10D20": 8.0, "h_D20": 12.0}

# Ref: Kotha et al. (2020)
...
def get_distance_coefficients_2(kind, c3, c3_epsilon, C, imt, sctx):
    ...
    C3_R = C3_REGIONS_AVGSA[imt] if kind.startswith(
        "avgsa") else C3_REGIONS[imt]
    for i in range(1, 6):
        idx = sctx.region == i
...
class KothaEtAl2020(GMPE):
    ...
    def get_mean_and_stddevs(self, ctx: np.recarray, imt: IMT):
        """
        ...
        fp = get_distance_term(self.kind, c3, self.c3_epsilon,
                               C, ctx, imt)
        mean = (get_magnitude_scaling(C, ctx.mag) + fp
