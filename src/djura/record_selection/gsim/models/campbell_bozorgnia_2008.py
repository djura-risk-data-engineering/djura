...
from .. import const
from ..imt import PGA, PGV, PGD, CAV, SA, IMT
...
def _compute_distance_term(C, ctx: Context):
    """
...
def _compute_hanging_wall_term(C, ctx: Context):
    """
...
def _compute_imt1100(C, ctx: Context, get_pga_site=False):
    """
...
def _compute_shallow_site_response(C, ctx: Context, pga1100):
    """
...
def _compute_style_of_faulting_term(C, ctx: Context):
    """
...
def _get_hanging_wall_distance_term(ctx: Context):
    """
...
def _get_stddevs(kind, C, ctx: Context, pga1100, sigma_pga):
    """
...
class CampbellBozorgnia2008(GMPE):
    ...
    #: average horizontal :attr:`const.IMC.GMRotI50`
