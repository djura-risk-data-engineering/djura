...
from .abrahamson_2014 import get_epistemic_sigma
from .. import const
...
class ChiouYoungs2014(GMPE):
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        if repr(imt) == "PGA":
            ...
            sig, tau, phi = pga_sig, pga_tau, pga_phi
        else:
            ...
            mean = np.where(imt_mean < pga_mean, pga_mean, imt_mean) \
                if repr(imt).startswith("SA") and imt.period <= 0.3 \
                else imt_mean
            mean += (self.sigma_mu_epsilon*get_epistemic_sigma(ctx))

            sig, tau, phi = imt_sig, imt_tau, imt_phi
...
class ChiouYoungs2014ACME2019(ChiouYoungs2014):
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        mean = _get_mean(ctx, C, ln_y_ref, exp1, exp2)
