...
def _get_style_of_faulting_term(C, ctx):
    ...
    fflt_m = np.clip(ctx.mag - 4.5, 0.0, 1.0)
...
class BozorgniaCampbell2016(GMPE):
    ...
    *Earthquake Spectra*, 32(2), 979-1004.
    ...
    def get_mean_and_stddevs(self, ctx: Context, imt: IMT):
        ...
        mean = get_mean_values(self.SJ, self.sgn, C, ctx)
...
class BozorgniaCampbell2016HighQ(BozorgniaCampbell2016):
    """
    Implements the Bozorgnia & Campbell (2016) NGA-West2 GMPE,
    for the low attenuation (high quality factor) coefficients.
    """

    def __init__(self):
        super().__init__(sgn=1)


class BozorgniaCampbell2016LowQ(BozorgniaCampbell2016):
    """
    Implements the Bozorgnia & Campbell (2016) NGA-West2 GMPE,
    for the high attenuation (low quality factor) coefficients.
    """
...
class BozorgniaCampbell2016AveQJapanSite(BozorgniaCampbell2016):
    """
    Implements the Bozorgnia & Campbell (2016) NGA-West2 GMPE,
    for the average attenuation (low quality factor) coefficients,
    for the case in which the "Japan" shallow site response term is activited.
    """
...
class BozorgniaCampbell2016HighQJapanSite(BozorgniaCampbell2016):
    """
    Implements the Bozorgnia & Campbell (2016) NGA-West2 GMPE,
    for the high attenuation (low quality factor) coefficients,
    for the case in which the "Japan" shallow site response term is activited.
    ...
    def __init__(self):
        super().__init__(SJ=1, sgn=1)


class BozorgniaCampbell2016LowQJapanSite(BozorgniaCampbell2016):
    """
    Implements the Bozorgnia & Campbell (2016) NGA-West2 GMPE,
    for the low attenuation (low quality factor) coefficients,
    for the case in which the "Japan" shallow site response term is activited.
    """
