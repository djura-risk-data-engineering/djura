...
def make_prediction(dynamic_ductility: float, period: float, site: str,
    ...
    if site.lower() == "rock":
        phi = 1 + 1 / (10 * period - dynamic_ductility * period) - 1 / \
            (2 * period) * np.exp(-3 / 2 * pow(np.log(period) - 3 / 5, 2))
    elif site.lower() == "soft-soil":
        phi = 1 + 1 / (12 * period - dynamic_ductility * period) - 2 / \
    ...
    strength_ratio = (dynamic_ductility - 1) / phi + 1
